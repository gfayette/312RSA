 ...
 * signing messages, verifying signatures, and obtaining private keys by factoring weak moduli.
 *
...
public class RSA {
    ...
    public BigInteger[] keysFromPrimes(BigInteger p, BigInteger q, BigInteger minPubKey) {
        System.out.println("Generating keys for " + name + " with minimum public" + " key size " + minPubKey + "...");

        BigInteger[] keys = generateKeys.generateKeyPair(p, q, minPubKey);
        publicKey = keys[0];
        privateKey = keys[1];
        modulus = keys[2];
    ...
    public BigInteger[] keysFromMinVal(BigInteger minPrime, BigInteger minPubKey) {
        System.out.println("Generating primes greater than " + minPrime + "...");
        long start = System.currentTimeMillis();

        BigInteger p = generatePrimes.generatePrime(minPrime);
        Random r = new Random();
        BigInteger qMin = p.multiply(BigInteger.valueOf((long) (100.0 + 900.0 * r.nextDouble())));
    ...
    public BigInteger encryptRSA(BigInteger message, BigInteger pubKey, BigInteger mod) {
    ...
    public BigInteger encryptStringRSA(String messageString, BigInteger pubKey, BigInteger mod) {
        System.out.println(name + " is converting the shared key \"" + messageString + "\" to numeric...");

        BigInteger message = convertNumeric.convertToNumeric(messageString);
    ...
    public BigInteger decryptRSA(BigInteger cipherText) {
        ...
        BigInteger M = exponentMod.exponentMod(cipherText, privateKey, modulus);
    ...
    public String decryptStringRSA(BigInteger cipherText) {
        ...
        System.out.println(name + " is converting " + M + " back to a" + " String...");
    ...
    public BigInteger signMessage(BigInteger message) {
        ...
        BigInteger signature = exponentMod.exponentMod(message, privateKey, modulus);
    ...
    public boolean verifySignature(BigInteger signature, BigInteger message, BigInteger publicKey, BigInteger modulus) {
        System.out.println(name + " is verifying the signature ...");
        if (exponentMod.exponentMod(signature, publicKey, modulus).compareTo(message) == 0) {
            System.out.println(signature);
            ...
            System.out.println(signature);
            System.out.println("Is NOT a valid signature for the " + "message");
            System.out.println(message);
            System.out.println();
            return false;
        }
    ...
    public BigInteger[] getPrivateKey(BigInteger pubKey, BigInteger mod) {
        System.out.println("Cracking modulus...");
        long start = System.currentTimeMillis();
        BigInteger[] primes = factorModulus.fermats(mod);
        BigInteger p = primes[0];
        BigInteger q = primes[1];
        BigInteger privateKey = generateKeys.generatePrivateKey(p, q, pubKey);
        double elapsedSeconds = (System.currentTimeMillis() - start) / 1000.0;
        ...
        System.out.println("p = " + p);
        System.out.println("q = " + q);
        System.out.println("The private key corresponding to the " + "public key " + pubKey + " is:");
        System.out.println(privateKey);
        System.out.println("It took " + elapsedSeconds + " seconds to" + " find the private key");
        System.out.println();
    ...
    public BigInteger encryptStringXOR(String messageString) {
        System.out.println(name + " is converting the message \"" + messageString + "\" to numeric...");
