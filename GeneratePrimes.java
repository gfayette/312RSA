...
public class GeneratePrimes {
    public BigInteger[] generatePrimes(BigInteger min, int numPrimes) {
        ...
        while (n.compareTo(new BigInteger("4")) == -1 && primesFound < numPrimes) {
    ...
    private boolean simpleTest(BigInteger n) {
        ...
        if (n.mod(BigInteger.TWO).compareTo(BigInteger.ZERO) == 0 || n.mod(new BigInteger("3")).compareTo(BigInteger.ZERO) == 0) {
            return false;
        }

        BigInteger i = new BigInteger("5");
        while (i.multiply(i).subtract(BigInteger.ONE).compareTo(n) == -1) {
            if (n.mod(i).compareTo(BigInteger.ZERO) == 0 || n.mod(i.add(BigInteger.TWO)).compareTo(BigInteger.ZERO) == 0) {
                return false;
            }
